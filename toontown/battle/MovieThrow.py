...
def __throwPie(throw, delay, hitCount):
    ...
    if hitSuit:
        ...
        if kbbonus > 0:
            ...
            moveTrack = Sequence(Wait(0.4), LerpPosInterval(suit, 0.6, pos=suitPos, other=battle))
    ...
    for i in xrange(numTargets):
        ...
        if hitSuit:
            ...
            if groupHitValues.count(0) == len(groupHitValues):
                ...
                singleSuitResponseTrack = Sequence(Wait(tPieHitsSuit - 0.8), Func(MovieUtil.indicateMissed, suit, 1.0))
